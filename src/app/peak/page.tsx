    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="rounded-3xl bg-slate-950 p-6 text-white shadow-xl sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-5">
          ...
          <div className="text-right text-sm text-slate-300"><p>{new Intl.DateTimeFormat("zh-TW", { dateStyle: "full", timeZone: "Asia/Taipei" }).format(new Date())}</p><p className="mt-1">同步：{formatDate(receivedAt)} {snapshotStale ? "· 已過期" : ""}</p><form action="/api/peak/v1/logout" method="post"><button className="mt-3 rounded-lg border border-slate-600 px-3 py-1.5 text-xs hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-300">登出</button></form></div>
        </div>
