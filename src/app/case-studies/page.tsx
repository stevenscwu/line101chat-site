import type { Metadata } from "next";

import { CTASection, DemoCard } from "@/components/cards";
import { SectionHeading } from "@/components/section-heading";
import { demoCases } from "@/data/site";

export const metadata: Metadata = {
  title: "成功案例 / Demo｜LINE101Chat",
  description:
    "查看 LINE101Chat 的 RAG 文件問答助理與 Indonesian ⇄ Traditional Chinese LINE 翻譯助理 Demo 案例。",
  alternates: { canonical: "/case-studies" },
};

export default function CaseStudiesPage() {
  return (
    <main>
      <section className="bg-slate-50 px-5 py-16 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <SectionHeading
            eyebrow="成功案例 / Demo"
            title="成功案例 / Demo"
            description="以下是目前可展示的兩個工作原型：一個是文件來源導向的 RAG 問答助理，一個是 LINE 裡的印尼文繁體中文雙向翻譯助理。"
          />
          <div className="mt-8 grid gap-5 lg:grid-cols-2">
            {demoCases.map((demo) => (
              <DemoCard key={demo.title} {...demo} />
            ))}
          </div>
        </div>
      </section>

      <section className="bg-white px-5 py-16 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-3">
          {[
            ["PoC 重點", "先選一個可衡量的場景，例如招生 FAQ、SOP 查詢或跨語言溝通。"],
            ["驗證方式", "使用真實問題測試回答準確度、來源引用、LINE 使用體驗與維護流程。"],
            ["下一步", "若 PoC 成效明確，再擴充文件、使用者、權限與維護機制。"],
          ].map(([title, body]) => (
            <article key={title} className="rounded-lg border border-slate-200 bg-slate-50 p-5">
              <h2 className="text-lg font-black text-slate-950">{title}</h2>
              <p className="mt-3 text-sm leading-7 text-slate-600">{body}</p>
            </article>
          ))}
        </div>
      </section>

      <CTASection
        title="想用你的文件做第一個 Demo？"
        body="準備一份乾淨文件與真實問題，我們可以協助判斷是否適合做 Starter PoC。"
        label="取得 PoC 評估"
      />
    </main>
  );
}
